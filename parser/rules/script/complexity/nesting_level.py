...
from ....models import , PODModelProjectContext, PMDModel
...
class ScriptNestingLevelRule(Rule):
    ...
    def visit_pod(self, pod_model: PODModel):
        ...
        for field_path, field_value, field_name, line_offset in script_fields:
            if field_value and len(field_value.strip()) > 0:
                yield from self._check_nesting_level(field_value, field_name, pod_model.file_path, line_offset)

    def _analyze_script_file(self, script_model):
        """Analyze standalone script files for nesting levels."""
        try:
            yield from self._check_nesting_level(script_model.source, "script", script_model.file_path, 1)
