...
class ScriptComplexityRule(Rule):
    ...
    def visit_pmd(self, pmd_model: PMDModel, context=None):
        ...
        for field_path, field_value, field_name, line_offset in script_fields:
            if field_value and len(field_value.strip()) > 0:
                yield from self._check_complexity(field_value, field_name, pmd_model.file_path, line_offset, context)
    ...
    def visit_pod(self, pod_model: PodModel, context=None):
        ...
        for field_path, field_value, field_name, line_offset in script_fields:
            if field_value and len(field_value.strip()) > 0:
                yield from self._check_complexity(field_value, field_name, pod_model.file_path, line_offset, context)

    def _analyze_script_file(self, script_model):
        """Analyze standalone script files for cyclomatic complexity."""
        try:
            yield from self._check_complexity(script_model.source, "script", script_model.file_path, 1, None)
        except Exception as e:
            print(f"Warning: Failed to analyze script file {script_model.file_path}: {e}")

    def _check_complexity(self, script_content, field_name, file_path, line_offset=1, context=None):
