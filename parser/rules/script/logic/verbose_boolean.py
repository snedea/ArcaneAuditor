...
class ScriptVerboseBooleanCheckRule(Rule):
    ...
    def visit_pod(self, pod_model: PodModel):
        ...
        for field_path, field_value, field_name, line_offset in script_fields:
            if field_value and len(field_value.strip()) > 0:
                yield from self._check_verbose_boolean_patterns(field_value, field_name, pod_model.file_path, line_offset)

    def _analyze_script_file(self, script_model):
        """Analyze standalone script files for verbose boolean patterns."""
        try:
            yield from self._check_verbose_boolean_patterns(script_model.source, "script", script_model.file_path, 1)
