...
class ScriptRuleBase(Rule, ABC):
    ...
    def analyze(self, context) -> Generator[Finding, None, None]:
        ...
        for pod in context.pods.values():
            yield from self._analyze_pod(pod)
        
        # Analyze script files
        for script in context.scripts.values():
            yield from self._analyze_script(script)
    ...
    def _analyze_pmd(self, pmd_model: PMDModel, context) -> Generator[Finding, None, None]:
        ...
        yield from self._analyze_fields(pmd_model, script_fields)
    
    def _analyze_pod(self, pod_model: PodModel) -> Generator[Finding, None, None]:
        """Analyze POD file for script fields."""
        script_fields = self.find_pod_script_fields(pod_model)
        yield from self._analyze_fields(pod_model, script_fields)
    
    def _analyze_script(self, script_model: ScriptModel) -> Generator[Finding, None, None]:
    ...
    def _analyze_fields(self, model, script_fields: List[Tuple[str, str, str, int]]) -> Generator[Finding, None, None]:
        """Analyze script fields from a model."""
        for field_path, field_value, field_name, line_offset in script_fields:
            if field_value and field_value.strip():
                yield from self._check(field_value, field_name, model.file_path, line_offset)
